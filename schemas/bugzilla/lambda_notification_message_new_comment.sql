DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `lambda_notification_message_new_comment`(IN `notification_type` varchar(255), IN `bz_source_table` varchar(240), IN `notification_id` varchar(255), IN `created_datetime` datetime, IN `unit_id` smallint(6), IN `case_id` mediumint(9), IN `case_title` varchar(255), IN `created_by_user_id` mediumint(9), IN `message_truncated` varchar(255), IN `case_reporter_user_id` mediumint(9), IN `old_case_assignee_user_id` mediumint(9), IN `new_case_assignee_user_id` mediumint(9), IN `current_list_of_invitees` mediumtext, IN `current_status` varchar(64), IN `current_resolution` varchar(64), IN `current_severity` varchar(64))
